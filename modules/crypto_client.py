...
def connect(curr, xcurr, conn=None):
...
def trans_exist(conn, txid):
    ...
    #except Exception as e:
        ...
        pass
    return res
# если связь прервана то заново 
def trans_exist_try(conn, txid, curr, xcurr):
    try:
        return trans_exist(conn, txid)
    except:
        conn_new = connect(curr, xcurr)
        return trans_exist(conn_new, txid)
...
def get_unspents(conn, conf_from=None, vol=None, addrs=None, accs=None, xcurr=None):
    ...
    tab = []
    #print 'conf_from:', conf_from
    lUnsp = conn.listunspent(conf_from)
    #print '\n+++++++++++\n lUnsp:'
    #for r in lUnsp: print 'lUnsp':', r
    ...
    for unsp in lUnsp:
        ...
        #print 'sumChange + sumGen + sumReceive', sumChange, sumGen, sumReceive, '=', sumChange + sumGen + sumReceive
...
def send_to_many(db, curr, xcurr, sends_in, tx_fee_in=None, conn_in=None):
    ...
    txfee = float(tx_fee_in or xcurr.txfee or 0.0001)
    if vol < txfee * 30:
    ...
    res = None
    addr = None
    conf_for_send = xcurr.conf_hard # xcurr.conf_true - нельзя труе - не успевает накопиться и баланса вечно не хватает
    # наберем входы неиспользованные на этот объем монет
    lus, amo_u = get_unspents(cn, conf_for_send, vol, addr)
    print 'get_unspents for conf:', conf_for_send, amo_u #, lus
    transFEE = calc_txfee(cn, lus, sends, txfee)
    print 'calc transFEE:', transFEE

    # заново наберем чтобы комиссию включить
    lus, amo_u = get_unspents(cn, conf_for_send, vol + transFEE, addr)
    ...
    # TXID - тут будет не правильный без подписи
    trans_json = cn.decoderawtransaction(tx_str)
    print '\n\n decoderawtransaction(tx_str) - [%s]' % trans_json

    res = cn.signrawtransaction( tx_str )
    print 'signrawtransaction:'##, res -> { 'hex': '...', 'complete': True }
    '''

    ## теперь подпишем трнзакцию
    res = cn.signrawtransaction( tx_str )
    ...
    # теперь у нее надо взять TXID - он будет такой же при отсылке
    tx_hex = res['hex']
    trans_json = cn.decoderawtransaction(tx_hex)

    ## запомним ее в нашей базе а то если пошлем а потом ошибка то надо чтобы мы это поняли и не слали 2 раза выплаты балансов
    ## короче кладем этот TXID + JSON в нашу базу данных
    ##raw_id = db.xcurrs_raw_trans.insert(xcurr_id = xcurr.id, txid = res[u'txid'], tx_hex = '%s' % res, confs=-1 )
    raw_id = db.xcurrs_raw_trans.insert(xcurr_id = xcurr.id, txid = trans_json['txid'], tx_hex = '%s' % trans_json, confs = 0 )
    ## COMMIT !!!
    log_commit(db, 'send_to_many', '%s: %s' % (curr.abbrev, sends_in))
    ...
    try:
        ...
        code = res.get('error')
        code = code and code.get('code') or -991
        db.xcurrs_raw_trans [ raw_id ].update_record ( confs = code )
        log(db, 'send_to_many', 'error sendrawtransaction: %s' % res)
        return { 'error': 'ERROR: sendrawtransaction %s' % (e), 'pars': {'lus': lus, 'sends': sends} }

    if type(res) != type(u' '):
        ## если маленький размер пула то он непонимает входы трнзакции и пишет ошибку:
        #### {u'id': 1, u'result': None, u'error':
        ######   {u'message': u'Error: The transaction was rejected! This might happen if some of
        ####    the coins in your wallet were already spent, such as if you used a copy 
        ####    of wallet.dat and coins were spent in the copy but not marked as spent here.',
        #### u'code': -4}}

        # запишем что она не послана
        code = res.get('error')
        code = code and code.get('code') or -992
        db.xcurrs_raw_trans [ raw_id ].update_record ( confs = code )
        log(db, 'send_to_many', 'sendrawtransaction: %s' % res)
        return { 'error': 'ERROR: sendrawtransaction %s' % (res), 'pars': {'lus': lus, 'sends': sends} }

    print 'sensed as txid:', res
    db.xcurrs_raw_trans [ raw_id ].update_record ( confs = 1 )

    return { 'txid': res, 'tx_hex': tx_hex, 'tx_json': trans_json, 'txfee': transFEE, 'db.xcurrs_raw_trans': raw_id }
