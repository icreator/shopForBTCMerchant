...
def send_to_many():
    import db_common
    import crypto_client
    if len(request.args) ==0: return 'send_to_many/LTC?Lc7nSnWdhp1RU9kCDZS8gRCV1WBAMupkYy=0.01&Lc7nSnWdhp1RU9kCDZS8gRCV1WBAMupkYy=0.02'
    curr, xcurr, ecurr = db_common.get_currs_by_abbrev(db, request.args[0])
    if not curr or not xcurr: raise HTTP(503, 'ERROR: xcurr [%s] not found' % request.args[0] )

    if not request.vars or len(request.vars)==0: HTTP(500, 'list ={}' )

    res = crypto_client.send_to_many(db, curr, xcurr, request.vars)
    print res
    return '%s' % res
